...
　　**转正（仅免费试用集群）：**点击该按钮，会弹出就去哪转正对话，提示用户试用集群将转为正式集群，并提示计费价格。
...
　　**管理工具：**Hadoop和Spark等应用具有丰富的原生管理工具，点击各个工具链接，能够直接进入管理界面，进行相关管理和查询。用户通过此接口进入管理工具界面，无需使用SSH Tunnel方式进入。管理工具包括ResourceManager、NameNode、JobHistory、Ganglia、Oozie和Spark，针对临时集群和常驻集群略有不同，详情请参考[监控指南](jian_kong_zhi_nan.md)
